...
class ScraperConfigManager:
    ...
    def load_config(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Loaded {len(self.targets)} scraper targets from {self.config_path}")
            return True
    ...
    def _validate_config(self) -> bool:
        ...
        required_sections = ['global_settings', 'categories', 'websites', 'extraction_rules']
        ...
        required_global = ['rate_limit_delay', 'max_retries', 'timeout_seconds', 'user_agent']
            ...
            for site_config in sites:
                try:
                    target = ScraperTarget(
                        ...
                        rate_limit=site_config.get('rate_limit', self.global_settings.rate_limit_delay),
                    ...
                    logger.warning(f"Invalid target configuration for {site_config.get('name', 'unknown')}: missing {e}")
                    continue
    
    def get_enabled_targets(self, category: str = None, priority: str = None) -> List[ScraperTarget]:
        ...
        if category:
            ...
            if category in self.config['websites']:
                site_names = [site['name'] for site in self.config['websites'][category]]
    ...
    def get_keywords(self, keyword_type: str = None) -> List[str]:
        ...
        # Return all keywords combined
        all_keywords = []
        for keywords in extraction_rules.values():
    ...
    def passes_quality_filters(self, content: str, metadata: Dict[str, Any] = None) -> bool:
        ...
        relevance_threshold = filters.get('relevance_scoring', {}).get('min_score_threshold', 0)
    ...
    def get_optimal_scraper_order(self, target_name: str, url: str = None) -> List[str]:
        ...
        if url and (url.endswith(('.rss', '.xml')) or '/feed/' in url):
            if strategy.get('rss_available'):
                return ['feedparser'] + [s for s in strategy['fallbacks'] if s != 'feedparser']
        
        # Check for JavaScript-heavy sites
        if strategy.get('js_heavy'):
            return ['playwright', 'crawl4ai'] + [s for s in strategy['fallbacks'] if s not in ['playwright', 'crawl4ai']]
