...
class SimpleJuniorMiningScraper:
    ...
    async def extract_data(self, url, content):
        ...
        try:
            ...
            print(f"   📊 Found {len(companies)} companies, {len(news)} news items, {len(stock_data)} stock entries")
    ...
    def extract_companies(self, text, soup):
        ...
        for pattern in symbol_patterns:
            matches = re.findall(pattern, text)
            for match in matches:
                if 2 <= len(match.split('.')[0]) <= 5:  # Valid symbol length
                    found_symbols.add(match)
        
        # Create company records
        company_list = list(found_companies)[:20]  # Limit to 20
        symbol_list = list(found_symbols)[:20]
        
        for i, company in enumerate(company_list):
            companies.append({
                'name': company,
                'symbol': symbol_list[i] if i < len(symbol_list) else None,
                'source': 'text_extraction'
            })
        
        # Add additional symbols without matched names
        for symbol in symbol_list[len(company_list):]:
            companies.append({
        ...
        for selector in news_selectors:
            try:
                ...
                for element in elements:
                    ...
                    if len(text) > 10 and len(text) < 200 and text not in found_headlines:
    ...
    def calculate_mining_relevance(self, text):
        ...
        mining_keywords = [
            ...
            'tsx', 'tsxv', 'financing', 'merger', 'acquisition'
        ]
    ...
    def merge_site_data(self, site_data):
        ...
        for company in site_data['companies']:
            ...
            for existing_company in self.results['companies_found']:
                if (company.get('symbol') and company['symbol'] == existing_company.get('symbol')) or \
                   (company.get('name') and company['name'] == existing_company.get('name')):
    ...
    async def generate_summary(self):
        ...
        successful_sites = len([s for s in self.results['sites_scraped'] if s['success']])
    ...
    async def save_results(self):
        ...
        with open(summary_file, 'w', encoding='utf-8') as f:
            ...
            f.write(f"Target Sites: {self.results['performance_metrics']['sites_attempted']}\n")
            f.write(f"Successful: {self.results['performance_metrics']['sites_successful']}\n")
            f.write(f"Success Rate: {self.results['performance_metrics']['success_rate']:.1f}%\n\n")
            
            f.write("RESULTS SUMMARY:\n")
            f.write(f"- Companies Found: {self.results['performance_metrics']['total_companies']}\n")
            f.write(f"- News Items: {self.results['performance_metrics']['total_news_items']}\n")
            f.write(f"- Data Quality Score: {self.results['performance_metrics']['data_quality_score']}/100\n")
            f.write(f"- Duration: {self.results['performance_metrics']['scraping_duration']:.1f}s\n\n")
            ...
            if self.results['news_items']:
                ...
                sorted_news = sorted(self.results['news_items'], key=lambda x: x['relevance_score'], reverse=True)
